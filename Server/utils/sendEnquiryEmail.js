...
export const sendEnquiryEmail = async ({
  ...
  const html = `
    <div style="font-family: Arial, sans-serif; line-height: 1.6;">
      ...
      <p><b>Status:</b> ${listing?.status || listing?.project_status || "-"}</p>
      <p><b>Price Range:</b> ${formatPrice(listing?.price_start)} - ${formatPrice(listing?.price_end)} ${listing?.currency || ""}</p>
      <p><b>Location:</b> ${listing?.location || listing?.project_location || "-"}</p>
      <p><b>City:</b> ${listing?.project_city || listing?.city_data?.name || "-"}</p>
      <p><b>Expected Completion:</b> ${listing?.expected_completion_date || "-"}</p>
      <hr/>
      <h3>Developer Details</h3>
      <p><b>Developer:</b> ${listing?.developer_name || "-"}</p>
      <p><b>Developer Email:</b> ${listing?.developer_email || "-"}</p>
      <p><b>Developer Phone:</b> ${listing?.developer_phone || "-"}</p>
      <p><b>Developer Website:</b> ${listing?.developer_website || "-"}</p>
      <hr/>
